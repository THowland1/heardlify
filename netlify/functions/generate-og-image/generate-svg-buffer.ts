...
function buildRoundedRectangle(
  coords: ICentredViewbox,
  borderRadius: number,
  fill: string
) {
  const { minX, minY, width, height } =
    convertCentredCoordinatesToViewBox(coords);
  return /* html */ `
  ...
  buildRoundedRectangle(
    {
      ...
      width:
        padding + spotifyLogoWidth + padding + approximateTextWidth + padding,
    },
    20,
    '#444'
  )
...
export const generateSVGBuffer = async (text: string) => {
  let img = sharp(Buffer.from(buildSVG(text)));
